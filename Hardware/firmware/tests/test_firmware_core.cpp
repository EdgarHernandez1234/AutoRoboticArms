        ...
        if (status == PARSE_SUCCESS) {
            if (!is_safe) {
                ...
                if (g_consecutive_valid_frames >= 3) {
                    ...
                    mock_uart_transmit_byte('H'); // Still building trust
                }
            } else {
                watchdog_reset();
                mock_uart_transmit_byte('K');
                g_consecutive_valid_frames = 3;
            }

            if (is_safe || g_consecutive_valid_frames >= 3) {
            ...
            g_consecutive_valid_frames = 0; // Destroy trust
        }
    }
...
std::string generate_valid_frame(int j0, int j1, int j2) {
    char payload[32];
    sprintf(payload, "DRV,%d,%d,%d", j0, j1, j2);
    uint8_t checksum = 0;
...
void test_cyber_physical_watchdog_trip(void) {
    ...
    // Advance mock time past the 3000ms threshold instantly
    ...
    // Assert 2: The system locks the PCA9685 at g_cached_angles (holding torque)
    // Ticks for 90 degrees = 307
    assert(read_mock_pca9685_ticks(0) == 307);

    // Feed a valid frame while halted
    std::string frame = generate_valid_frame(180, 180, 180);
    feed_serial_string(frame.c_str());
    for(size_t i=0; i<frame.length(); i++) execute_super_loop_step();
